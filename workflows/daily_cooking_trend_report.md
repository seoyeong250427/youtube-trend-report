...
- **오늘의 콘텐츠 주제 추천 3~5개**: 오늘 데이터에서 실제로 반복되는 패턴(키워드, 제목 후킹 방식, 포맷)을 근거로 — 이건 "알고리즘/트렌드에 편승하는" 주제들
- **예시 대본 5개**: 위 추천과 연결되는 60초 기준 4단 구조(0-3초 훅 → 3-10초 상황 → 10-50초 과정 → 50-60초 반응) 대본
- **시청자에게 실질적으로 도움되는 주제 5개** (신규): 위와는 별도로, 트렌드/후킹이 아니라 **실제 요리 지식·노하우로 시청자에게 진짜 도움이 되는 주제**를 5개 뽑는다. 예: 조리 원리(왜 이 순서로 해야 하는지), 식재료 보관법, 도구 관리, 식비/낭비 줄이는 실용 정보, 흔한 실수 교정 등. 데이터에서 억지로 끌어오지 말고, 요리에 대한 일반 지식 중 오늘 수집된 트렌드 주제(예: 오늘 인기 키워드)와 자연스럽게 연결되는 것을 우선한다.
- **위 5개에 대한 대본 5개** (신규): 같은 60초 4단 구조로, 정보 전달이 명확하게 되도록 작성 (후킹은 궁금증으로 시작하되, 과정 단계에서 "왜 그런지" 이유를 반드시 한 줄 넣는다)
