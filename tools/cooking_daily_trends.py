...
--keywords로 다른 테마의 키워드 설정 파일을 지정하면, 같은 Tool로 다른 테마(예: 집밥 전용)의
일일 리포트도 만들 수 있다. 새 테마마다 Tool을 새로 만들지 않는다.
"""
...
DEFAULT_KEYWORDS_CONFIG_PATH = ROOT_DIR / "tools" / "config" / "cooking_keywords.json"
...
def main():
    ...
    parser.add_argument("--keywords", default=str(DEFAULT_KEYWORDS_CONFIG_PATH), help="키워드 설정 JSON 경로")
    ...
    config = load_config(Path(args.keywords))
